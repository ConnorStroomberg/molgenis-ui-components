    ...
    <nav class="navbar navbar-toggleable-md navbar-default">
      ...
      <div :class="{collapse:collapse, 'navbar-collapse':collapse}" id="navbarNavDropdown">
          ...
          <template v-for="(item, index) in menu.items">
            ...
            <template v-if="item.type == 'PLUGIN'">
              ...
              <li class="nav-item" :class="{active:isSelectedPlugin(item.id)}">
                <a v-if="showNavBarLogo(index)" class="navbar-brand" :href="'/menu/main/'+item.href"><img
                  :src="navBarLogo"></a>
                <a v-else class="nav-link" :href="'/menu/main/'+item.href">{{item.label}}</a>
              </li>
            </template>
            <template v-else>
        ...
        <ul class="nav navbar-nav">
          ...
          <li class="nav-link">
            <a class="nav-link navbar-right btn btn-secondary" :href=helpLink.href
          ...
          <li v-if="authenticated"class="nav-link">
            <form id="logout-form" class="navbar-form navbar-right" method="post" action="/logout">
              <button id="signout-button" type="button" class="btn btn-primary" v-on:click="logout">Sign out
              </button>
            </form>
          </li>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      showNavBarLogo: function (index) {
        return index === 0 && this.navBarLogo
      },
